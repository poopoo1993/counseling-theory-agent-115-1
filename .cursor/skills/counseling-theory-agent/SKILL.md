...
- Practice themes: keys of `PRACTICE_THEMES` (Chinese labels in UI). Difficulty UI: `初階` / `中階` / `進階`. 初階 = live plan beside chat + in-chat sentence notes. 中階 = in-chat sentence notes only. 進階 = end-of-session review only.
