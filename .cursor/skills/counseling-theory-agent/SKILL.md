...
3. **Three in-session Gemini engines, then a separate post-session assessment.** Planner is used for **practice** (to generate the case) and for **進階** experience. Experience **初階／中階** skip the planner and use analyzer + chatbot only. Analyzer (each student turn) is internal JSON **except** 初階／中階, which show a simple `turn_review` in the chat. **初階** also shows a side plan (`student_guide` + `example_replies`) that must follow the current talk; do **not** repeat `turn_review` on the side. The chatbot must not teach, score, or name techniques. **進階** shows formative review only after the session ends.
...
5. **`ChatLogs.content_raw` is append-only.** Preserve full text, including parenthetical nonverbal cues. Do not rewrite past turns when re-scoring. 初階旁欄「當下的想法與判斷」只存在 `session_state.coach_thoughts`，不得寫入 `ChatLogs` 或逐字稿。
...
- Practice themes: keys of `PRACTICE_THEMES` (Chinese labels in UI). Difficulty UI: `初階` / `中階` / `進階`. 初階 = live plan beside chat + in-chat sentence notes + a side thought box (not written to `ChatLogs`). 中階 = in-chat sentence notes only. 進階 = end-of-session review only.
