...
3. **Three in-session Gemini engines, then a separate post-session assessment.** Planner is used for **practice** (to generate the case) and for **進階** experience. Experience **初階／中階** skip the planner and use analyzer + chatbot only. Chatbot replies first; the analyzer is a **side-channel** after the visible line for **初階／中階** (`turn_review` in chat). **進階** skips mid-session analyze and shows formative review only after the session ends. **Practice 初階** also shows a side plan (`student_guide` + `example_replies`) that must follow the current talk; do **not** repeat `turn_review` on the side. **Practice 中階** keeps the side thought box and in-chat sentence notes, but **no** plan panel. **Experience 初階** shows observer notes (`student_guide` only: what the counselor is doing now) and no next-line examples. The chatbot must not teach, score, or name techniques.
...
| Path | Role |
| --- | --- |
| `app.py` | Streamlit UI: whitelist OTP login, student chat, `session_flow.tick()`, continuation, teacher dashboard, ZIP export |
...
| `src/llm_pipeline.py` | Plan / chat / analyze / thought / eval job builders; analysis fail-open |
| `src/session_flow.py` | SessionPhase + per-browser Gemini job queue (`tick` = one job per rerun) |
...
1. Student path: API key gate → `session_flow` queue (`plan` if needed, then opening `chat` on the **next** rerun) → live **reply-first** (`chat` then optional `analyze`) → `pending_finalize` (eval+snapshot batch). Do not `finish_session` until that batch (or decline purge) succeeds.
2. Practice start uses planner JSON (includes case fields) before the first AI turn. Continuation reuses `case_data`, `counseling_plan`, `chat_analysis` (including 初階 `student_guide`), `latest_snapshot`, and the thread’s ChatLogs in the chat UI. **Do not** generate a new `public_opening` first line when prior turns exist. **Do not** show `unfinished_issues` / next-session goals to the student; discovering session goals is the student’s task. Dialogue and planner omit those agenda fields. 初階 continuation skips the planner so the last live plan can carry over. Threads still store last 6 `recent_turns` as fallback if ChatLogs were purged. Threads upsert **once at start** (`in_progress`) and **once at end** (`active` or `closed`). ChatLogs still append every turn.
3. Safety: `detect_pii` blocks send; `detect_immediate_risk` stops simulation, writes `RiskEvents`, sets `safety_stopped`, and closes the thread so it is not stuck `in_progress`. Keep crisis patterns **specific** so classroom suicide-prevention talk does not trip them (`tests/test_safety.py`).
...
- Transcripts: UTF-8-SIG; filename `theory_agent_transcript_<session_id>.txt`. Dialogue, analysis, thought, eval, and snapshot prompts send the full transcript (Gemini long-context retrieval); do not add a summarizer pass.
...
- **Student-browser Gemini router:** Streamlit Cloud builds prompts but does **not** call Gemini. A persistent hidden iframe POSTs `generateContent` and returns `{results:[{request_id, text, model, latency_ms, error}]}`. Cache by stable `call_id`. Raise `GeminiRouterPending` (a `BaseException`) until the iframe finishes. Never write the key to Sheets or logs. Prompt text is visible in DevTools; that is accepted for this classroom app. **Do not remount the iframe per call** (`key=ct_gemini_router`). Streamlit allows that key **once per rerun**; `session_flow.tick()` therefore runs **at most one Gemini job** (eval+snapshot is one batch). If plan just returned, wait for the next rerun before opening chat. A student turn **replies first**. Analyzer runs **afterwards** for 初／中階 `turn_review` / side plan and must not block the visible line. If the student sends again, chat jumps to the front of the queue; **do not drop** the pending analyze. 進階 skips mid-session analyze. Practice and experience both use `pending_finalize`; `finish_session` happens only after eval+snapshot (or decline purge) succeeds so a slow iframe can retry. Threads are not upserted every turn.
...
- **Context window:** Chat, analyze, thought, eval, and snapshot all receive the **full** prior+current transcript. Do not slice to 14 turns or add a second summarizer just to fit context. Gemini is used for long-context retrieval. `AppConfig.recent_context_turns` default `0` means unlimited; prompts currently send everything.
- **Stored `temperature`:** `new_session` writes `0.4`; live calls use 0.55 (chatbot), 0.65 (planner), 0.1 (analyzer/eval/snapshot).
- **`Sessions` vs `Threads`:** `school_name` is on the in-memory session and on Threads, not on the Sessions headers—extra keys are dropped on append. Threads upsert at **start** (`in_progress`, not listed for 續談) and **end** (`active` if the process is kept, `closed` after decline / anonymous / safety-stop). Do not upsert Threads every chat turn. `list_threads` is `active` only.
