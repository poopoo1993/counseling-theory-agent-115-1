...
- **Student-browser Gemini router:** Streamlit Cloud builds prompts but does **not** call Gemini. `generate_via_browser` sends the job (including the student’s API key) to a hidden iframe that POSTs `generateContent` and returns `{request_id, text, model, latency_ms, error}`. Cache by stable `call_id` across reruns; raise `GeminiRouterPending` (a `BaseException`) until the iframe finishes so `except Exception` fail-open paths do not swallow the wait. Never write the key to Sheets or logs. Prompt text is visible in DevTools; that is accepted for this classroom app.
