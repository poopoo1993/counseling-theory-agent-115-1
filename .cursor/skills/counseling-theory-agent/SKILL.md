...
- Practice themes: keys of `PRACTICE_THEMES` (Chinese labels in UI). Difficulty UI: `初階` / `中階` / `進階`. Practice 初階 = live plan + examples beside chat + in-chat sentence notes + a side thought box (not written to `ChatLogs`). Experience 初階 = observer notes beside chat (no examples, no thought box) + in-chat sentence notes. 中階 = in-chat sentence notes only. 進階 = end-of-session review only. Experience never scores the student-as-client.
- `participant_id` is `P-` + 12 hex from `sha256(salt:email)`. Salt is a secret; never commit it. Anonymous research rows use `P-ANON-` + 12 random hex (`new_anonymous_participant_id`); never hash the email.
...
- **Browser refresh:** Streamlit `session_state` is wiped on F5. Login is restored from a hashed `sid` query token in SQLite `AuthSessions` (TTL 12h). Gemini API Keys may be remembered in **browser localStorage only** after a successful test, never in SQLite. Logout deletes the login token, not the browser key list. Skip `AuthSessions` in research ZIP export.
