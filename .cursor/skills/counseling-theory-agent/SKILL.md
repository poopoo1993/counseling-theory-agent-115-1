...
5. **`ChatLogs.content_raw` is append-only.** Preserve full text, including parenthetical nonverbal cues. Do not rewrite past turns when re-scoring. 實作初階旁欄「當下的想法與判斷」只存在 `session_state.coach_thoughts`，不得寫入 `ChatLogs` 或逐字稿。體驗（學生當個案）不顯示想法框。
