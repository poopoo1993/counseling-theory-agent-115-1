...
2. Practice start uses planner JSON (includes case fields) before the first AI turn. Continuation reuses `case_data`, `counseling_plan`, `chat_analysis`, `latest_snapshot`, and the thread’s ChatLogs in the chat UI. **Do not** generate a new `public_opening` first line when prior turns exist. **Do not** show `unfinished_issues` / next-session goals to the student; discovering session goals is the student’s task. Dialogue and planner omit those agenda fields. Threads still store last 6 `recent_turns` as fallback if ChatLogs were purged. Threads upsert **once at start** (`in_progress`) and **once at end** (`active` or `closed`). ChatLogs still append every turn.
