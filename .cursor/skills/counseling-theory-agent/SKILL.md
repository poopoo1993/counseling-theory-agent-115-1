...
5. **`ChatLogs.content_raw` is append-only during a session.** Preserve full text, including parenthetical nonverbal cues. Do not rewrite past turns when re-scoring. Exception requested by product: after **experience** 「結束晤談」, ask whether the student consents to research use; if they decline, **delete that session’s ChatLogs** (and do not persist quoted Assessments/SkillEvents). Practice transcripts stay. 實作初階旁欄「當下的想法與判斷」只存在 `session_state.coach_thoughts`，不得寫入 `ChatLogs` 或逐字稿。體驗（學生當個案）不顯示想法框。
