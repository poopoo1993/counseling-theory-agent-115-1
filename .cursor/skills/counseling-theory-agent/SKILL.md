...
2. Practice start uses planner JSON (includes case fields) before the first AI turn. Continuation reuses `case_data`, `counseling_plan`, `chat_analysis`, `latest_snapshot`, and last 6 `recent_turns`, then re-plans. Threads upsert **once at start** (`in_progress`) and **once at end** (`active` or `closed`). ChatLogs still append every turn.
