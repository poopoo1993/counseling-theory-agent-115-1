...
- 對話與評量分開呼叫 Gemini，避免 AI 在晤談中突然教答案。
- 雙向續談：可延續同一位 AI 個案，也可延續同一位 AI 諮商師。
- 括弧非語言訊息，例如「（視線移開）」；原始文字完整保留。
- `@hcu.edu.tw` OTP 登入，另支援教師測試白名單。
- 學生自行輸入 Gemini API Key；Key 僅存於瀏覽器工作階段，不寫入資料表或逐字稿。
- Google Sheets 後台：身分對照、Sessions、ChatLogs、Threads、Assessments、SkillEvents、TeacherGrades、Settings、RiskEvents。
...
1. 依 [Google Sheets 設定](docs/GOOGLE_SHEETS_SETUP.md) 建立空白試算表及服務帳戶。
2. 依 [部署操作手冊](docs/DEPLOYMENT_GUIDE.md) 上傳 GitHub 並設定 Streamlit Secrets。
